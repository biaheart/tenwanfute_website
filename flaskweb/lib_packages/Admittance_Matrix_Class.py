...
class Admittancematrix:  # 建立节点导纳矩阵类
    ...
    def add(self, node1, node2, resistance, reactance, total_b):  # 增加支路的支路追加法
    ...
    def minus(self, node1, node2, resistance, reactance, total_b):  # 删除支路的支路追加法
        if resistance == 0 and reactance == 0:
            return
        impedance = complex(resistance, reactance)  # 计算阻抗
        admittance = 1 / impedance  # 计算导纳
    ...
    def generate_matrix(self, nodelist1, nodelist2, resistance, reactance, total_b, conductance, susceptance):
        for i in range(len(nodelist1)):
            self.add(nodelist1[i], nodelist2[i], resistance[i], reactance[i], total_b[i])
